...
The optional =hook= cell belongs to the dependent pair. A blank hook has no dependency. Existing single-target hooks remain valid: =BASE-tail-closed= waits for a successful close of =BASE=, =BASE-head-filled= waits for its played head, and the legacy bare form =BASE= also means =BASE-tail-closed=.

Use =all(...)= when every listed lifecycle event must occur before the dependent pair can proceed, or =any(...)= when one listed event is sufficient. Targets are comma-separated and expressions cannot be nested:

#+begin_example
| name    | hook                                    |
|---------+-----------------------------------------|
| CHILD_A | BASE-tail-closed                        |
| CHILD_B | all(BASE-head-filled,HEDGE-tail-closed) |
| CHILD_C | any(BASE-tail-closed,HEDGE-tail-closed) |
#+end_example

Fan-out needs no special syntax: several dependent rows may use the same =BASE-tail-closed= target. Fan-in is expressed on one dependent row with =all(...)= or =any(...)=. For repeated pairs, dependency evidence is fresh per downstream attempt: ALL requires every target again, while ANY requires one new target. Events before the attempt was armed or outside the dependent pair's =tps_run= window do not count.

Dependency expressions are compiled before runtime startup. Missing pairs, self-links, duplicate targets, malformed expressions, and dependency cycles fail preflight before feeds or exchange execution start. Pair names and lifecycle suffixes are case-sensitive; the =all= and =any= keywords are case-insensitive.
...
The optional =rFunc= column names a repeat-adjustment function registered at runtime. Leave it blank for fixed repeats. Use =name: x,y= to pass numeric arguments, for example =head_offset_toggle: 3,8=. Define operator-specific policies in =orders/rfunc.py=; that file is ignored by git because it may encode live strategy. The tracked =kolabi/bot/repeat_adjustment.py= module owns only the registry, loader, context, argument parsing, and safety validation. A local rFunc file should import =register_repeat_adjustment=, define pure Python functions with the signature =(OrderPairSpec, RepeatAdjustmentContext) -> OrderPairSpec=, and register each operator-facing name. Startup/preflight validates the rFunc name, arguments, and any registered policy-specific constraints before the bot loop starts. Set =KOLABI_STRATEGY_RFUNC=/path/to/rfunc.py= to load a different local file. The local head-offset examples include =head_offset_once: x= and =head_offset_toggle: a,b= for =L=, =S=, =SL=, =LT=, and =MT= heads; they change the next =hPrice= offset and preserve any =hDelta= trigger-to-limit offset.
...
- =orders/demo_rfunc.org= documents rFunc argument syntax for head price offset policies.
