...
The optional =rFunc= column names a repeat-adjustment function registered at runtime. Leave it blank for fixed repeats. Define operator-specific policies in =orders/rfunc.py=; that file is ignored by git because it may encode live strategy. The tracked =kolabi/bot/repeat_adjustment.py= module owns only the registry, loader, context, and safety validation. A local rFunc file should import =register_repeat_adjustment=, define pure Python functions with the signature =(OrderPairSpec, RepeatAdjustmentContext) -> OrderPairSpec=, and register each operator-facing name. Set =KOLABI_STRATEGY_RFUNC=/path/to/rfunc.py= to load a different local file.
